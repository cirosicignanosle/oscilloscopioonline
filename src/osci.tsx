...
// Stili di stampa per salvare/stampare la pagina Studenti in PDF
function ensurePrintStyles() {
  if (document.getElementById("print-styles")) return;
  const style = document.createElement("style");
  style.id = "print-styles";
  style.innerHTML = `@media print {
    nav, footer, a[href="#/docente"], [href="#/docente"] { display: none !important; }
...
function waveformSample(type: string, t: number, p: any) {
  ...
  switch (type) {
    ...
    case "sum2": {
      // 1° + 2° armonica (peso 0.5) normalizzata a picco 1
      const raw = Math.sin(w * t + phase) + 0.5 * Math.sin(2 * w * t + 2 * phase);
      const NORM = 1.299038105676658; // 3*sqrt(3)/4
      y = amplitude * (raw / NORM);
      break;
    }
    case "sum135": {
      // Serie dispari 1 + (1/3)·3 + (1/5)·5, pesi 1/k – normalizzata a picco 1
      const raw = Math.sin(w * t + phase) + (1/3) * Math.sin(3 * w * t + 3 * phase) + (1/5) * Math.sin(5 * w * t + 5 * phase);
      const NORM = 14/15; // ≈0.9333333333
      y = amplitude * (raw / NORM);
      break;
    }
    case "sum1357": {
      // Serie dispari 1 + 1/3 + 1/5 + 1/7 – normalizzata a picco 1
      const raw = Math.sin(w * t + phase)
                + (1/3) * Math.sin(3 * w * t + 3 * phase)
                + (1/5) * Math.sin(5 * w * t + 5 * phase)
                + (1/7) * Math.sin(7 * w * t + 7 * phase);
      const NORM = 0.930088244804217; // picco numerico
      y = amplitude * (raw / NORM);
      break;
    }
    case "chirp": {
      // Sweep non-armonico: f(t) = f0 + k*t (k proporzionale alla freq di base)
      const k = 0.6 * frequency; // sweep rate
      const phaseInst = 2 * Math.PI * (frequency * t + 0.5 * k * t * t) + phase;
      y = amplitude * Math.sin(phaseInst);
      break;
    }
...
function ScopeCanvas({ shared, view, canvasId = "scope-canvas" }: any) {
  ...
  useEffect(() => {
    ...
    // assi centrali più evidenti (verticale al centro, orizzontale centrata sull'offset V)
      ...
      <div className="grid grid-cols-2 gap-3 text-sm">
        <div className="col-span-2">
          ...
          <select className="w-full bg-slate-800 border border-slate-700 rounded-xl p-2" value={ch.waveform} onChange={(e) => set("waveform", (e.target as HTMLSelectElement).value)}>
            ...
            <option value="sum2">Armonica: Somma 1+0.5·2</option>
            <option value="sum135">Armonica: 1+3+5 (1/k)</option>
            <option value="sum1357">Armonica: 1+3+5+7 (1/k)</option>
            ...
            <option value="chirp">Non armonica: Chirp (sweep)</option>
...
function TeacherPage({ shared, setShared }: any) {
  ...
  const randomize = (level: "base" | "intermedio" | "avanzato") => {
    ...
    const forms = ["sine", "square", "triangle", "saw", "rectified", "am", "fm", "sum2", "sum135", "sum1357", "chirp"];
...
function StudentPage({ shared }: any) {
  ...
  const doCheck = () => {
    ...
    let totalErrors = 0; let channelsCorrect = 0; Object.values(res.perChannel).forEach((r: any) => { totalErrors += r.errors || 0; r.ok = (r.errors || 0) === 0; if (r.ok) channelsCorrect++;});

    // conteggio risposte inserite (campi numerici compilati)
    let filledCount = 0;
    for (const id of [1,2,3]) {
      const m: any = (measures && (measures as any)[id]) || {};
      ["vmax","vmin","vpp","period","freq"].forEach(k => { const v = parseFloat(m[k]); if (isFinite(v)) filledCount++; });
    }

    // mappatura voti in base alla tabella
    let voto = 2; let descr = "Prestazione gravemente insufficiente; necessita supporto";
    if (filledCount === 0) { voto = 1; descr = "Nessuna misura valida inserita"; }
    else if (channelsCorrect === 3 && totalErrors === 0 && datiMancanti === 0) { voto = 10; descr = "Tutte e tre le onde corrette, nessuna imprecisione"; }
    else if (channelsCorrect === 3) { voto = 9; descr = "Tutte e tre le onde corrette (anche in casi complessi)"; }
    else if (channelsCorrect === 2) { voto = 8; descr = "Due onde corrette su tre"; }
    else if (channelsCorrect === 1) { voto = 7; descr = "Una onda corretta su tre"; }
    else if (totalErrors <= 2) { voto = 6; descr = "Correttezza parziale nel caso più semplice"; }
    else if (totalErrors <= 5) { voto = 5; descr = "Imprecisioni ricorrenti nell'esercizio base"; }
    else if (totalErrors <= 8) { voto = 4; descr = "Due o tre errori significativi"; }
    else if (totalErrors <= 12) { voto = 3; descr = "Molti errori; difficoltà marcate"; }

    setCheckResult({ ...res, voto, descr, erroriCount: totalErrors, datiMancanti, channelsCorrect, filledCount });
  };
  ...
  return (
    <div className="p-6 text-slate-100">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold">Pagina Studenti</h1>
        <div className="flex items-center gap-2">
          <button onClick={() => window.print()} className="px-3 py-2 rounded-xl border border-slate-700 bg-slate-800 hover:bg-slate-700 text-sm">
            Stampa / Salva PDF
          </button>
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-6 items-start">
        ...
        <aside className="w-full lg:w-80 p-5 rounded-2xl border border-slate-700 bg-slate-900/40">
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            ...
            <NumberField label="s/div" value={view.sPerDiv} step={0.0001} min={0.0001} onChange={(v: number) => set("sPerDiv", v)} />            <NumberField label="Offset V (V)" value={view.vOffset} step={0.1} onChange={(v: number) => set("vOffset", v)} />
      ...
      {checkResult && (
        <div className="text-sm mt-4">
          ...
          <div className="mt-2"><strong>Voto: {checkResult.voto} / 10</strong><div className="text-slate-400 text-xs mt-1">{checkResult.descr}</div></div>
